...
封包從 tap100i0 出來：
  ...
  2) bridge forward（原生 stateful 規則）
     ...
     ├── 按 .fw 檔案中的出現順序評估
     └── 全部未命中 → 放行（沒有隱式 default policy）
```

> pvefw-neo **不讀** VM [OPTIONS] 的 `policy_in` / `policy_out` —— 全部改成
> per-port。使用者若想要「預設 drop」語意，自己在對應 iface 尾端加一條
> `IN DROP -i netN` / `OUT DROP -i netN` 當 catch-all 即可。
...
| **沒有隱式 default policy** | 沒規則 = 雙向 accept。VM [OPTIONS] 的 `policy_in` / `policy_out` 不會被讀取；需要預設 drop 就在該 iface 尾端自己加一條 catch-all rule |
| **VM-level 設定不使用** | PVE 的 VM 級 `dhcp`/`macfilter`/`ipfilter`/`ndp`/`radv`/`policy_*` 一律忽略。對應語意改由 per-port `@neo:` tag 表達（見 §5.2 sugar 總表），只有 `enable` 切換仍然被尊重 |
...
table bridge pvefw-neo {
    ...
    chain vm_tap100i0_in {
        ...
        # User-authored IN DROP catch-all (replaces the old policy_in=DROP)
        drop
    }
...
# 留給 PVE WebUI 存放，pvefw-neo 一律忽略（對應功能走 per-port @neo: tag）。
...
IN DROP                                  # 自訂 catch-all（取代 policy_in=DROP 語意）
OUT Finger(DROP) -i net0 # @neo:ipspoof 10.0.0.100/32
```

### 9.2 解析流程

1. 解析 `[OPTIONS]` → `enable` / `log_level_in` / `log_level_out`（其他 key 靜默忽略）
...
4. 解析 `[RULES]`，對每條規則：
   ...
   c. 分類：
      - Finger sugar carrier + `@neo:` sugar tag → 展開成對應的 NOTRACK / stateful 規則
      - 正常規則 + `@neo:noct` → 放入 raw chain
      - 正常規則 + `@neo:srcmac`/`@neo:dstmac`/`@neo:vlan`/`@neo:ether` → 加上對應 L2 match
      - 正常規則 + `@neo:rateexceed` (僅限 noct + DROP) → 設定 rate limit
...
CT2004 自己的 per-iface catch-all DROP 擋掉。
