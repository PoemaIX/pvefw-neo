...
| Tag | 效果 |
|---|---|
| `@neo:disable` | 關閉 pvefw-neo 對此 port 的管理，該 port 上其他規則全部忽略，流量直接放行。<br>等同 PVE 的 "port-level firewall disable"(之所以不能用 PVE GUI 那個旗標：打勾之後 PVE 會建立 fwbr會導致不相容) |
...
| Tag | 效果 |
|---|---|
| `@neo:noct` | 在 **conntrack 之前**執行，per-packet 匹配<br>別名：`@neo:stateless` |
...
| `@neo:srcmac notin <mac1,mac2>` | 此規則只套用到 source MAC 不等於 `<mac>` 的封包 |
...
使用方式和 Extension rules 相同（Finger 骨架 + comment）。
...
IP 清單填在 Source 欄位或者 comment 參數位(擇一)  
（注意：PVE Source 欄位不接受 v4/v6 混寫。  
...
|---|---|---|---|
| `out` | `DROP` | `+ipspoof_vm100_net0_v4` | `@neo:noct @ether arp op {request,reply}`（ARP 保護）|
| `out` | `DROP` | `+ipspoof_vm100_net0_v4` | `@neo:noct`（IPv4 保護）|
| `out` | `DROP` | `+ipspoof_vm100_net0_v6` | `@neo:noct`（IPv6 保護）|

> IPv6 ipset 永遠自動包含 `fe80::/64`（link-local）和 `::`（DAD），
