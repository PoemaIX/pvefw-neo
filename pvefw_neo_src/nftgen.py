...
class NftRenderer:
    ...
    def render(self):
        ...
        # Flush our tables first
        lines.append("# Flush existing pvefw-neo tables")
        for devname in sorted(self._netdev_rules):
            lines.append(f"table netdev pvefw-neo-{devname}")
            lines.append(f"delete table netdev pvefw-neo-{devname}")
        # Always try to delete the bridge table (in case previous run had one)
        ...
        # Isolation: list of devs with isolated=True
        isolated_devs = [
            nd.devname for nd in self.rs.netdevs.values()
            if nd.devname in self.devnames and nd.isolated
        ]
