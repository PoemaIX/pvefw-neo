...
5. **pvefw-neo ignores every VM-level toggle PVE exposes in
   `VM → Firewall → Options`** (`policy_in`, `policy_out`, `dhcp`, `macfilter`,
   `ipfilter`, `ndp`, `radv`). Only the `Firewall` enable switch on that tab is
   honored. Equivalent behaviour is per-port via `@neo:` tags — which is what
   makes (4) possible in the first place. Want default-drop on a specific NIC?
   Add an explicit `IN DROP -i netN` rule at the end of that NIC's chain.
---
